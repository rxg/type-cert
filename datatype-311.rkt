...
(provide define-type type-case lambda-memocontract)
  ...
  (syntax-parse
    ...
    [(_ datatype:id (~and (~seq immut ...) (~optional #:immutable))
        [variant:id (field:id field/c:expr)
                    ...
                    (~and (~seq when-clause ...)
                          (~or* (~seq #:when when-body:expr) (~seq)))]
        ...)
            ...
            (quasisyntax/loc stx
              (begin
                ...
                (begin
                  ...
                  ;; check if the #:when clause holds
                  ;; this should always be true when called externally,
                  ;; but is needed to check the post condition of mutators
                  (define (check-variant variant)
                    (make-check-variant-body ([field (variant-field f:variant)] ...) when-clause ...)
                    )
                ...
                #,@(if mut?
                       #'[(define set-variant-field!
                            (lambda-memocontract (v nv)
                                                 (contract (->i ([f:variant f:variant?] [field (the-field/c)]) [_ void] #:post (f:variant) (f:check-variant f:variant))
                                                           set-variant*-field!
...
(define-syntax (->i-when stx)
  (syntax-case stx ()
    [(_ fields dom range)
     #'(->i dom range)]
    [(_ fields dom #:when when-clause range)
     #'(->i dom #:pre fields when-clause range)]))

(define-syntax (make-check-variant-body stx)
  (syntax-case stx ()
    [(_ bindings)
     #'#t]
    [(_ bindings #:when when-clause)
     #'(let bindings when-clause)]))
